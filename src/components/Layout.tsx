...
type NavItem = {
  to: "/dashboard" | "/monitoring" | "/uploads" | "/alerts" | "/incidents" | "/users" | "/settings";
  label: string;
...
const nav: NavItem[] = [
  ...
  { to: "/uploads", label: "Video Uploads", icon: UploadCloud },
...
export default function Layout() {
  ...
  useEffect(() => {
    const off = onNewAlert((a) => {
      toast.error(`New ${a.alert_type}`, {
        description: `${a.camera_name} — ${(a.confidence * 100).toFixed(0)}%`,
      });
      if (sound) {
        try {
          const ctx = new (
