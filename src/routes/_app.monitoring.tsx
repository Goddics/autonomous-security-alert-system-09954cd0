...
function MonitoringPage() {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      grad.addColorStop(0, "#0b1220"); grad.addColorStop(1, "#1a2540");
      ...
      ctx.beginPath(); ctx.arc(x, y - 50, 18, 0, Math.PI * 2); ctx.fill();
      ctx.fillRect(x - 22, y - 30, 44, 60);
      // bounding box if running
      if (running) {
        ctx.strokeStyle = "rgba(239, 68, 68, 0.9)";
        ctx.lineWidth = 2;
        const bx = x - 36, by = y - 76, bw = 72, bh = 116;
  ...
  return (
    <div className="p-8">
      <PageHeader title="Live Monitoring" subtitle="YOLOv8n inference stream with bounding box overlay." />
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-card border border-border rounded-xl overflow-hidden">
            <div className="aspect-video bg-black relative">
              ...
              {running && (
                <div className="absolute top-3 left-3 flex items-center gap-2 px-2 py-1 rounded bg-threat/90 text-white text-xs font-medium">
                  <span className="h-2 w-2 rounded-full bg-white animate-pulse" /> REC · LIVE
                </div>
              )}
              {!running && (
            ...
            <div className="p-4 flex flex-wrap items-center gap-3 border-t border-border">
              <div className="flex items-center gap-2">
                ...
                <select
                  value={camId} onChange={e => setCamId(Number(e.target.value))}
              ...
              <div className="ml-auto text-xs text-muted-foreground">Model: YOLOv8n · 30 FPS · 640×640</div>
            </div>
          </div>
        </div>
              ...
              {feed.map(a => (
                <li key={a.id} className="text-sm border-l-2 border-threat/60 pl-3">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {a.camera_name} · {(a.confidence * 100).toFixed(0)}% · {new Date(a.timestamp).toLocaleTimeString()}
                  </div>
                </li>
              ))}
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            ...
            <ul className="space-y-2 text-sm">
              <li className="flex items-center justify-between">
                <span className="flex items-center gap-2"><Cpu className="h-4 w-4 text-muted-foreground" /> Inference</span>
                <span className="text-success text-xs">Healthy</span>
              </li>
              <li className="flex items-center justify-between">
                <span className="flex items-center gap-2"><Wifi className="h-4 w-4 text-muted-foreground" /> WebSocket</span>
                <span className="text-success text-xs">Streaming</span>
              </li>
              <li className="flex items-center justify-between">
                <span className="flex items-center gap-2"><CamIcon className="h-4 w-4 text-muted-foreground" /> Cameras Online</span>
                <span className="text-xs">{cams.length}/{cams.length}</span>
              </li>
            </ul>
          </div>
        </div>
